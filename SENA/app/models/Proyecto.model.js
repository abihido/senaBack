...
Proyecto.create = (newProyecto, result) => {
  ...
  Proyecto.getAll = result => {
    sql.query("SELECT name ,requisitos,descripcion,active,categoria,idOrganizacion, presupuesto,actividad,idProyectos,puestos,creator,avatarCreator,url  FROM proyectos", (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
        return;
